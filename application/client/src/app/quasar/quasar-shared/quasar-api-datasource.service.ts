...
export class QuasarApiDatasourceService {
  private _api;
   ...
   * @example this._api.url('auth', 'signin')
   * @param token
   * @param name
   * @returns string
   */
  url(token, name) {
    let selected = this._dataSources.map((ds: ApiDataSource, index) => {
  ...
  urlWith(token, name): any {
    let selected = this._dataSources.map((ds: ApiDataSource, index) => {
      ...
      if (generators.length > 0) {
        return generators
          .map((generator: GeneratorApiDataSource) => generator.callback)
          .shift();
      }
    });
    selected.map((result, index) => {
