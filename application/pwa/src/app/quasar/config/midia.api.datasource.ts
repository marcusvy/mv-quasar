...
export const MIDIA_API_DATASOURCE: ApiDataSource = {
  ...
  generators: [
    { name: 'audio:page', callback: (page = 1) => `/api/midia/audio/list/${page}` },
    { name: 'audio:id', callback: (id) => `/api/midia/audio/{id}` },
    { name: 'image:page', callback: (page = 1) => `/api/midia/image/list/${page}` },
    { name: 'image:id', callback: (id) => `/api/midia/image/{id}` },
    { name: 'video:page', callback: (page = 1) => `/api/midia/video/list/${page}` },
